...
const FileUploader: React.FC<FileUploaderProps> = ({
  ...
  const validateFile = (file: File): { valid: boolean; error?: string } => {
    ...
    if (allowedFileTypes.length > 0 && !allowedFileTypes.includes(file.type)) {
      return { 
        valid: false, 
        error: `File ${file.name} has unsupported format. Allowed types: ${allowedFileTypes.join(', ')}` 
      };
    }
    ...
    <div className="max-w-xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      >
        <input 
          ...
          multiple 
        />
        <svg xmlns="http://www.w3.org/2000/svg" className={`mx-auto h-12 w-12 ${dragActive ? 'text-blue-500' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
        </svg>
        ...
        <div className="mt-6">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-sm font-medium text-gray-700">
              Selected files ({files.length}{maxFiles > 0 ? `/${maxFiles}` : ''})
            </h3>
            {files.length > 1 && (
              <button 
                  ...
                  <div className="ml-3 flex-grow">
                    <div className="flex justify-between">
                      ...
                      <p className="text-xs text-gray-500">
                        {file.error ? 'Failed' : file.uploaded ? 'Complete' : `${file.progress}%`}
                      </p>
                    </div>
                    <p className="text-xs text-gray-500">{formatFileSize(file.size)}</p>
                    <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                      <div 
                        className={`h-1.5 rounded-full ${getProgressColor(file)}`} 
                        style={{width: `${file.progress}%`}}
                      ></div>
                    </div>
                  </div>
